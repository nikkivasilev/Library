...
resource "azurerm_resource_group" "rg" {
  name     = "${var.resource_group_name}nikkivasilev"
  location = var.resource_group_location
}

resource "azurerm_service_plan" "appsp" {
  name                = "${var.app_service_plan_name}-nikkivasilev"
...
resource "azurerm_linux_web_app" "appservice" {
  name                = "${var.app_service_name}-nikkivasilev"
  resource_group_name = azurerm_resource_group.rg.name
...
resource "azurerm_mssql_server" "sqlserver" {
  name                         = "${var.sql_server_name}-nikkivasilev"
  resource_group_name          = azurerm_resource_group.rg.name
  location                     = azurerm_resource_group.rg.location
  version                      = "12.0"
...
resource "azurerm_mssql_database" "sql" {
  name           = "${var.sql_database_name}nikkivasilev"
...
resource "azurerm_mssql_firewall_rule" "example" {
  name             = "${var.firewall_rule_name}nikkivasilev"
  server_id        = azurerm_mssql_server.sqlserver.id
